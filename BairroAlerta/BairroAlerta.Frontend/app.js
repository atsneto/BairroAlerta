...
*/
const API_BASE_URL = "http://localhost:5000"; // <<< ajuste aqui (sem /api)
const API_ALERTAS = `${API_BASE_URL}/api/alertas`;

const CENTER = { lat: -15.78, lng: -47.93, zoom: 13 }; // centro padrão (ajuste para sua cidade)
...
function addMarkerForAlert(a) {
  ...
  // se API devolve lat/lng, use; se não, gere perto do centro
  const lat = a.latitude ?? a.lat ?? null;
  const lng = a.longitude ?? a.lng ?? null;
  const coords = (lat && lng) ? { lat, lng } : randomNear(CENTER);

  const marker = L.marker([coords.lat, coords.lng], { icon }).addTo(markersLayer);
  const html = `<b>${escapeHtml(tipo)}</b><div style="margin-top:6px">${escapeHtml(a.descricao ?? a.descricao ?? '')}</div><small style="color:#999">${new Date(a.data).toLocaleString()}</small>`;
